  ...
  return (
    <div className={`wheel-section ${fbClass}`}>
      <div className="wheel-container">
        <button className="wheel-confirm-circle" onClick={handleConfirm} disabled={disabled} title={`Choose: ${selectedType}`}>
          &#10003;
        </button>
        <svg className="wheel-svg" viewBox="0 0 220 220" onClick={handleSvgClick}
          style={{ cursor: disabled ? 'default' : 'pointer' }}>
          <circle cx={cx} cy={cy} r={r + 5} fill="none" stroke="#2C2416" strokeWidth="2" opacity="0.12" />
          <g style={{
          ...
          <circle cx={cx} cy={cy} r="5" fill="#2C2416" opacity="0.18" />
        </svg>
      </div>
    </div>
  )
